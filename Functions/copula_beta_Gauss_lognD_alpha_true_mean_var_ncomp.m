function [pdf] = copula_beta_Gauss_lognD_alpha_true_mean_var_ncomp(xb,xg,beta_mean,beta_std,beta_min,beta_max,mu,sigma,rho,p)
...
pdf=pdf+p(n_comp)*reshape(y.*p1(:).*p2(:),numel(xb),numel(xg)); % joint PDF 
end
end
